...
SELECT
    *,
    ...
    CASE acquisition_channel
        ...
        -- ВКонтакте и Telegram названы отдельно, поэтому подпись
        -- обязана сказать, что здесь всё остальное: «Реклама в
        -- соцсетях» читалась как надмножество обоих. Коротко — потому
        -- что это подпись сектора круговой диаграммы, и длинная за
        -- край не помещается.
        WHEN 'social'        THEN 'Прочие соцсети'
