...
Скрипт генерирует столько дней, сколько задано в `days`, начиная с
`start_date`; эти и остальные параметры задаются в `growth_config.json`.
...
Продолжение после сбоя: `./run_downstream_pipeline.sh <номер дня>`, начиная
с дня, на котором произошёл сбой. Шаги дня повторяются безопасно: ingest
пропускается, если для дня есть манифест
`data/raw/_manifests/load_date=<день>/ingest_manifest.json`;
`transform.py` перезаписывает партиции дня; `load_to_clickhouse.py`
пропускает загруженный день и очищает незавершённый. Если ingest упал
посреди записи (raw-партиции есть, манифеста нет), партиции
`data/raw/*/load_date=<день>` удаляются вручную.
