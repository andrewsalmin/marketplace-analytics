...
DATASETS: dict[str, dict[str, str]] = {
    ...
    "order_stage_durations": {
        "phase": "p2",
        "note": "6.7 тайминги этапов переехали из виртуального датасета",
    },
    "orders_with_sequence": {
        "phase": "p2",
        "note": "6.7 номер заказа у покупателя переехал в витрину",
    },
    "customer_order_counts": {
        "phase": "p2",
        "note": "6.7 число заказов на покупателя переехало в витрину",
    },
...
def chart_patches() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "Тайминги воронки заказа (перцентили)": {
            ...
            "note": "P1-08 порядок перцентилей + 6.7 переезд на витрину",
...
#
# Названия каналов и причин отмены — длинные русские фразы, и внутрь
# сектора они не помещаются: подписи наезжали друг на друга. Вынос
# наружу это чинит, но упирается в другое — за краем круга подпись
# обрезается («Email-рассы…», «Яндекс. Д…»), потому что ширины у чарта
# всё равно нет.
#
# Поэтому роли разведены: название читается в легенде, где строка
# целая, а на секторе остаётся только доля — она короткая и помещается
# при любой ширине.
#
# Порог в 1% оставлен: доли мельче него подписывать некуда, а выносные
# линии от них превращаются в частокол.
PIE_LABELS = {
    # Внутрь, а не наружу. Снаружи подпись не наезжает на соседнюю, но
    # упирается в край контейнера и обрезается — «35,0…» вместо
    # «35,05%». Внутри сектора её обрезать нечему.
    "labels_outside": False,
    "label_line": False,
    "label_type": "percent",
    "show_labels": True,
    # Подписываются только заметные доли. У сектора в 3% внутри места
    # нет ни при каком радиусе, а вынести его наружу — вернуться к
    # обрезанию. Мелкие доли читаются в легенде и по наведению.
    "show_labels_threshold": 5,
    "show_legend": True,
    # Сбоку, а не сверху: семь названий вроде «Реклама в Telegram» в одну
    # строку не помещаются, и легенда сверху превращалась в листалку
    # «1/7» со стрелками — то есть переставала быть легендой. Столбцом
    # каждое имя занимает свою строку, а высоты у чарта хватает.
    "legendOrientation": "right",
    "legendType": "plain",
    # Легенда занимает правую треть, поэтому кругу оставлено меньше
    # места, чем по умолчанию.
    "outerRadius": 45,
}
