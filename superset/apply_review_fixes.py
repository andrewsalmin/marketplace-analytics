...
# Ровно то, что нужно анонимному посетителю, чтобы открыть дашборд
...
DATE_AXIS = {"x_axis_time_format": "%d.%m"}
...
DATASETS: dict[str, dict[str, str]] = {
    "customers": {
        "phase": "p0",
        "note": "FINAL + словарь каналов",
    },
    "orders": {
        "phase": "p0",
        "note": "FINAL + словарь статусов + is_mature",
    },
    "payments": {
        "phase": "p0",
        "note": "FINAL + словари статуса и способа оплаты",
    },
    "order_funnel_stages": {
        "phase": "p0",
        "note": "Воронка только по зрелым заказам",
    },
    ...
    "orders_with_customer_dim": {
        "phase": "p0",
        "note": "Признаки оплаты + русский канал",
    },
    "customer_first_order": {
        "phase": "p0",
        "note": "Признак зрелой когорты покупателей",
    },
    "first_order_delay_distribution": {
        "phase": "p0",
        "note": "Распределение задержки первого заказа",
    },
    "refund_processing_times": {
        "phase": "p0",
        "note": "Сырые длительности обработки рефанда",
    },
    "dq_reason_breakdown": {
        "phase": "p2",
        "note": "Разбивка DQ-ошибок с load_date",
    },
    "quarantine_volume": {
        "phase": "p2",
        "note": "Объём карантина по дням и сущностям",
    },
    "load_freshness": {
        "phase": "p2",
        "note": "Свежесть загрузки из _load_commits",
    },
    "payment_retries_distribution": {
        "phase": "p0",
        "note": "Попытки оплаты вместе с возмещёнными",
    },
    "orders_by_hour_dow": {
        "phase": "p1",
        "note": "Календарный знаменатель тепловой карты",
    },
    "customer_cohort_retention": {
        "phase": "p1",
        "note": "Retention с полной сеткой и нулями",
    },
}
...
def chart_patches() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "Изменение GMV": {
            ...
            "note": "GMV только по оплаченным + зрелый хвост",
        ...
        "Изменение среднего чека (AOV)": {
            ...
            "dataset": "orders_with_customer_dim",
            "note": "Средний чек по оплаченным заказам",
        ...
        "Время до первого заказа (дней с регистрации)": {
            "phase": "p0",
            "note": "Распределение вместо среднего по дате регистрации",
            ...
            "set": {
                ...
                "x_axis_sort_asc": True,
                # Хвост длинный и плоский: на линейной шкале первый
                # столбец в 2700 клиентов прижимает всё после
                # двадцатого дня к нулевой линии, и распределение
                # выглядит как один пик и пустота. Логарифм показывает
                # и пик, и то, что хвост тянется десятками клиентов до
                # конца окна наблюдения.
                "logAxis": True,
        ...
        "Конверсия: регистрация → первый заказ": {
            "phase": "p0",
            "note": "Явный признак заказа вместо даты",
            "description": (
        ...
        "Время обработки рефанда": {
            "phase": "p0",
            "note": "Перцентили по сырым длительностям вместо AVG",
        ...
        "Средний чек по городам (топ-10)": {
            ...
            "note": "Сортировка по метрике + оплаченные заказы",
            "set": {
                "metrics": [metric("Средний чек, ₽", AOV_PAID)],
        ...
        "Разбивка DQ-ошибок по типам": {
            "phase": "p0",
            "note": "Сортировка + подпись про срабатывания",
        ...
        "GMV по каналам привлечения": {
            ...
            "note": "Оплаченные заказы + сортировка + язык",
            "set": {
                "metrics": [metric("GMV, ₽", GMV_PAID)],
        ...
        "Воронка заказа": {
            "phase": "p1",
            "note": "Нативная воронка вместо баров с выпадающими подписями",
        ...
        "Число покупателей по городам": {
            ...
            "note": "Горизонтальные бары + сортировка",
        ...
        "Причины отмены заказов": {
            ...
            "dataset": "orders_with_customer_dim",
            "note": "Подписи вынесены наружу + словарь из витрины",
            "set": {
        ...
        "Изменение success rate оплаты": {
            "phase": "p1",
            "note": "Исход попытки не переписывается возвратом",
        ...
        "Тренд DQ-ошибок по сущностям": {
            "phase": "p1",
            "note": "Проценты + порог 5% + язык подписей",
            ...
            "description": (
                ...
                "заказам и платежам; номера в легенде — порядок загрузки, "
                "в котором ошибки и расходятся: за покупателем отклоняется "
                "его заказ, за заказом — платёж."
            ),
            "set": {
                # Деления шкалы приходятся на целые проценты, и «2,00%»
                # к «2%» ничего не добавляет. Знак процента при этом
                # остаётся на делениях, а не переезжает в подпись оси:
                # d3 не умеет умножить долю на сто, не напечатав знак, а
                # считать её в самой метрике — значит оставить всплывающую
                # подсказку с голым числом без единицы.
                "y_axis_format": ".0%",
        ...
        "Тайминги воронки заказа (перцентили)": {
            ...
            "note": "Порядок перцентилей + переезд на витрину",
        ...
        "Количество заказов по часам и дням недели": {
            ...
            "note": "Календарный знаменатель + понедельник сверху",
            "description": (
        ...
        "Retention по когортам регистрации (по неделям)": {
            ...
            "note": "Короткие подписи когорт + отсечение недели −1",
            "description": (
                "Строка — неделя, на которой покупатели зарегистрировались, "
                "столбец — сколько недель прошло с тех пор. В ячейке — доля "
                "когорты, сделавшая хотя бы один заказ на этой неделе. Ноль "
                "означает, что не вернулся никто; пустая ячейка — что неделя "
                "ещё не прожита до конца. Активность считается по созданному "
                "заказу."
            ),
            "set": {
                "metric": metric("Retention", "avg(retention_rate)"),
                "groupby": "cohort_week",
                # Единица измерения стоит в каждом делении, а не в
                # подписи оси: heatmap_v2 ключи x_axis_title и
                # y_axis_title принимает и не рисует — проверено на
                # стенде. Без слова столбцы «00», «01», «02» читаются
                # как код, а не как счётчик недель, поэтому повтор здесь
                # меньшее зло; сокращение делает его тише.
                #
        ...
        "Изменение числа заказов": {
            ...
            "dataset": "orders_with_customer_dim",
            "note": "Формат оси + сглаживание + заголовок",
        ...
        "Изменение числа новых и повторных заказов": {
            ...
            "note": "Формат оси + заголовок",
            "slice_name": "Динамика новых и повторных заказов",
            # Легенда шла по сумме, то есть «Повторный», потом «Новый» —
            # обратно порядку в заголовке чарта и обратно ходу событий.
            # По имени «Новый» встаёт первым.
            "set": {**DATE_AXIS, **SERIES_BY_NAME},
        },
        ...
        "Способы оплаты и статус": {
            "phase": "p2",
            "note": "Сортировка и подписи после перевода витрины на FINAL",
            "description": (
        ...
        "Распределение ретраев оплаты": {
            "phase": "p0",
            "note": "Попытки вместо ретраев, с учётом возмещённых",
...
# Форматы чисел на карточках. Правило одно на весь дашборд: то, что
# считают поштучно, показывается целиком; то, что не прочитать по
# цифрам, сокращается; доли — всегда с одним знаком. Разнобой в знаках
# после запятой создаёт ложное впечатление, будто одна доля измерена
# точнее другой.
...
# Порядок серий и легенды.
#
# Superset сортирует серии сам, и по умолчанию — по сумме, то есть по
# размеру, а не по смыслу. Легенда из-за этого читалась задом наперёд:
# «p99 → p90 → p50», «Повторный → Новый». Сортировка по имени
# возвращает смысловой порядок, и работает она ровно потому, что имена
# подобраны так, чтобы сортироваться правильно: «p50» < «p90» < «p99»,
# «Новый» < «Повторный».
#
# Там, где нужный порядок не алфавитный и не числовой, номер ставится в
# саму подпись — как у дней недели на тепловой карте («1 · Пн»), см.
# DQ_ENTITY.
SERIES_BY_NAME = {
...
# Разрез отбраковки: покупатели, заказы, платежи.
#
# Это порядок загрузки, и он же — направление, в котором ошибки
# расходятся: не загрузился покупатель — отклонится его заказ, за
# заказом отклонится платёж. Легенда, идущая в этом порядке, читается
# как ход пайплайна, а не как случайный список; алфавит поставил бы
# платежи раньше покупателей.
DQ_ENTITY = {
    "expressionType": "SQL",
    "label": "Сущность",
    "sqlExpression": (
        "CASE entity"
        " WHEN 'customers' THEN '1 · Покупатели'"
        " WHEN 'orders' THEN '2 · Заказы'"
        " WHEN 'payments' THEN '3 · Платежи'"
        " ELSE entity END"
    ),
}


# Цвет за подписью серии.
#
# Цвет серии Superset подбирает сам, по порядку появления подписи, и
# держит одно соответствие на весь дашборд. Порядок зависит от того,
# какой чарт отрисовался первым, то есть меняется от загрузки к
# загрузке, а подписей на дашборде больше, чем цветов в палитре, —
# поэтому две разные серии одного чарта получают один цвет и сливаются
# на гистограмме. Так «Оплачен» и «Возмещён» вышли одинаково голубыми,
# а «Новый», «Отправлен» и «Возвращён» — одинаково синими, все трое.
#
# Здесь цвет закреплён за подписью явно, и заодно получает смысл. Путь
# заказа — холодная шкала от светлого к тёмному, доставка — зелёный,
# как и успешный платёж; всё, что с пути свернуло, окрашено отдельно.
# «Возмещён» одинаков и там, где это статус платежа, и там, где это
# статус заказа: событие одно и то же.
LABEL_COLORS = {
    # Перцентили: оттенки одного цвета, потому что это одна величина на
    # трёх уровнях, а не три разные.
    "p50": "#8FD3E8",
    "p90": "#3AA6C4",
    "p99": "#1B6E87",
    # Статусы платежа.
    ...
    "Оплачен": "#3AA6C4",
    "Отправлен": "#1B6E87",
    "Готов к выдаче": "#454E7C",
    "Доставлен": "#5AC189",
    "Отменён": "#666666",
    "Возвращён": "#A868B7",
    # Первый заказ покупателя против повторного. Тёмный за первым:
    # его на графике мало, и светлый на белом фоне не читался.
    "Новый": "#454E7C",
    "Повторный": "#1FA8C9",
    # Причины отмены.
    "Не оплачен вовремя": "#1FA8C9",
    "Отменён покупателем после оплаты": "#454E7C",
    "Отменён покупателем до оплаты": "#5AC189",
    "Не забрали вовремя": "#FF7F44",
    # Сущности на графике отбраковки, в порядке загрузки.
    "1 · Покупатели": "#1FA8C9",
    "2 · Заказы": "#454E7C",
    "3 · Платежи": "#5AC189",
}
...
# Свежесть пайплайна на вкладке качества данных.
#
...
# Размещения, которые убираются с обзора: сами чарты остаются на своих
# тематических вкладках, а обзор не должен их повторять.
#
# CHART-explore-20-2 сюда не входит намеренно: это тренд GMV, и его место
# как раз на обзоре — оборот и есть главный вопрос к дашборду. Он стоит
# там под собственным ключом, см. OVERVIEW_TRENDS.
OVERVIEW_DUPLICATES = ["CHART-explore-2-2"]

# Обзор — короткий экран, а не свалка: четыре тренда, отвечающих на
# четыре разных вопроса, — сколько денег, сколько заказов, доходят ли
# платежи, много ли теряем на отменах. Всё остальное живёт на
# тематических вкладках и повторяться здесь не должно.
...
TAB_ROWS: list[tuple[str, str, str | None, list[tuple[str, str, int, int]]]] = [
    ...
    # Три чарта в ряд давали каждому по четверти экрана, и ни городам,
    # ни пирогу каналов этого не хватало: шестнадцать названий не
    # помещались, подписи секторов обрезались. Сверху теперь тренд
    # регистраций во всю ширину, под ним — два разреза этих же
    # регистраций, по половине на каждый.
    (
        "TAB-clients",
        "ROW-cl-1",
        None,
        [("CHART-cl-signups", "Динамика регистраций (среднее за 7 дней)", 12, 50)],
    ),
    (
        "TAB-clients",
        "ROW-cl-cities",
        "ROW-cl-1",
        [
            ("CHART-cl-cities", "Регистрации по городам", 6, 50),
            (
                "CHART-cl-channels",
...
class Runner:
    ...
    def _dashboard_look(self, metadata: dict[str, Any]) -> None:
        # Единая палитра плюс закреплённые цвета подписей.
        #
        # Рядом Superset держит свой кеш подобранных цветов
        # (map_label_colors), и в нём остаётся старое соответствие —
        # например, светло-голубой p99. Чистить его не нужно и вредно:
        # фронтенд наполняет кеш заново при каждой отрисовке, так что
        # прогон за прогоном показывал бы изменение на ровном месте.
        # Закреплённый цвет имеет приоритет над подобранным.
    ...
    def _native_filters(self, metadata: dict[str, Any]) -> None:
        # Фильтр периода применяется к собственной временной колонке
