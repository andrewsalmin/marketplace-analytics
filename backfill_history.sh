...
# error_rate (resolve_error_rate()): в первые недели после запуска
# площадки error_rate заметно выше базового значения ERROR_RATE
# (переменная ниже) — интеграции ещё «сырые» — и по экспоненте
# снижается до него. Поверх этого тренда — день-в-день случайный шум
# и редкие инциденты у отдельных сущностей (например, сорвался
# партнёрский API). Поэтому ERROR_RATE ниже — это уровень плато,
# к которому сходится кривая, а не буквальный error_rate конкретного
# дня.
#
# Использование:
#   ./backfill_history.sh         # с самого начала
#   ./backfill_history.sh 26      # возобновить с дня 26 (1-indexed) —
