...
`test_marts.py` проверяет текст SQL — что FINAL на месте, что дедлайны
не захардкожены. Здесь проверяется результат: что витрины считают то,
что обещают. Каждая из этих проверок соответствует ошибке, которая уже
была допущена и исправлена, — они существуют, чтобы та же ошибка не
вернулась молча.

Отличие от остальных тестов: нужен живой ClickHouse с загруженными
данными. Без него модуль пропускается — в CI данных нет и не будет.
Запускать там, где стоит хранилище:
...
PATH: на сервере креды уже лежат в ~/.clickhouse-client/config.xml, и
дублировать их в окружении незачем.
"""
...
def _query_via_client(sql: str) -> str:
...
def _query_via_http(sql: str) -> str:
...
def _query(sql: str) -> str:
    if shutil.which("clickhouse-client"):
        return _query_via_client(sql)
    return _query_via_http(sql)


def _available() -> bool:
    try:
        return _query(f"SELECT count() FROM {MARTS}.orders").strip().isdigit()
