...
# проверяет дубли client_id относительно уже обработанной истории
...
# отработал (см. backfill_month.sh) — эта стадия его не запускает.
#
# Использование:
#   ./run_downstream_pipeline.sh <clickhouse_password>          # с начала
#   ./run_downstream_pipeline.sh <clickhouse_password> 5        # с дня 5
#
# ВАЖНО про возобновление: в отличие от backfill_month.sh, здесь три
...
# этого дня он уже успешно отработал раньше. Если упало именно на
# load_to_clickhouse.py (ingest/transform уже прошли) — долечи этот
# конкретный день ОДНИМ вызовом load_to_clickhouse.py вручную, и
# возобновляй скрипт уже со СЛЕДУЮЩЕГО дня, а не с текущего.
set -euo pipefail
