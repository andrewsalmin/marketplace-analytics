...
def daily_marketplace_pipeline():
    ...
    # Дашборд правит сам себя раз в сутки — но только текстом, который
    # обязан следовать за данными.
    #
    # Речь о блоке «Что видно в данных»: там конверсия, доля отмен по
    # причинам и вклад Москвы, и каждое утверждение читатель может
    # сверить с графиком рядом. Данные прирастают каждую ночь этим самым
    # пайплайном, так что без пересчёта уже к утру блок утверждал бы с
    # точностью до десятой процента то, что из данных больше не следует.
    # Числа на карточках и графиках так не устаревают: они берутся из
    # витрин при каждом открытии.
    #
    # Скрипт идемпотентен и приводит дашборд к описанию в репозитории:
    # если ничего не разошлось, он печатает «0 изменений» и ничего не
    # пишет. Ночью меняться будет ровно этот текст.
    #
    # Последним и с retries=1: данные к этому моменту уже загружены и
    # проверены, поэтому недоступный Superset — повод повторить попытку,
    # а не повод считать ночь неудачной.
    #
    # depends_on_past=False, в отличие от остального DAG'а. Там правило
    # осмысленно: генератор накапливает клиентов между днями, transform
    # ищет дубли относительно прошлых загрузок — пропуск дня посреди
    # истории разошёлся бы тихо.
    #
    # Здесь наоборот. Задача читает текущее состояние витрин и пишет
    # текущий текст; что было вчера, ей не нужно. С унаследованным
    # depends_on_past одна неудачная ночь — перезапускался Superset —
    # заблокировала бы все последующие до ручного вмешательства, и
    # выводы замерли бы молча. Ровно то, ради предотвращения чего эта
    # задача и добавлена.
    @task(retries=1, depends_on_past=False, execution_timeout=timedelta(minutes=20))
    def refresh_dashboard() -> None:
        password = BaseHook.get_connection("clickhouse_default").password
        _run(
            [
                PYTHON_BIN, "superset/apply_review_fixes.py",
                "--phases", "p2",
                "--apply",
            ],
            # Выводы считаются по ClickHouse. Локальный clickhouse-client
            # берёт пароль из своего конфига сам, но на машине без него
            # остаётся запасной путь по HTTP — ему пароль нужен.
            env={"CLICKHOUSE_PASSWORD": password},
        )
