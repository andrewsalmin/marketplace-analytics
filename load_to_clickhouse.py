...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Один старый день в одиночку так перезагружать нельзя: "
            "версия строки в ReplacingMergeTree — ingested_at, то есть "
            "время загрузки, и заново загруженный день окажется свежее "
            "всех последующих. FINAL начнёт отдавать состояние заказа на "
            "тот день вместо нынешнего — молча, до тех пор пока не "
            "перезагрузишь и все дни после него."
        ),
    )
